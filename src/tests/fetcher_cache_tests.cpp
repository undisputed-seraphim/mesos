    ...
    const vector<CommandInfo>& commandInfos)
{
  ...
  vector<TaskInfo> tasks;
  for (const auto& commandInfo : commandInfos) {
...
TEST_F(FetcherCacheHttpTest, HttpCachedConcurrent)
{
  ...
  for (const auto& waypoint : fetchContentionWaypoints) {
...
TEST_F(FetcherCacheTest, RemoveLRUCacheEntries)
{
  ...
  // Fill up the cache
  for (const auto i : commandCreationPattern) {
    string commandFilename = "cmd" + stringify(i);
  ...
  for (const auto& cacheFile : cacheFiles.get()) {
