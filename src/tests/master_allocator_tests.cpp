...
TYPED_TEST(MasterAllocatorTest, MinAllocatableResources)
{
  ...
  for (const Resource& resourcesString : nonAllocatableAgentResources) {
  ...
  for (const Resource& resourcesString : allocatableAgentResources) {
    Future<SlaveRegisteredMessage> slaveRegisteredMessage =
      FUTURE_PROTOBUF(SlaveRegisteredMessage(), _, _);
  ...
  for (const auto& offer : offers.get()) {
