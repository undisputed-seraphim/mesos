    ...
    const Resources& checkpointedResources)
{
  ...
  for (const auto& resource : checkpointedResources) {
    ...
    const TOperation& operation)
{
  ...
  switch (operation.type()) {
    ...
    case TOperation::UNRESERVE: {
      for (const auto& reserved : operation.unreserve().resources()) {
    ...
    case TOperation::CREATE: {
      for (const auto& volume : operation.create().volumes()) {
...
Option<Error> validateAndUpgradeResources(Offer::Operation* operation)
{
  ...
  switch (operation->type()) {
    ...
    case Offer::Operation::LAUNCH_GROUP: {
      ...
      for (const auto& task : launchGroup->task_group().tasks()) {
        Option<Error> error = Resources::validate(task.resources());
        if (error.isSome()) {
          return error;
        }
