...
{
  for (const auto& [_, gauge] : operations_pending) {
    process::metrics::remove(gauge);
  }

  for (const auto& [_, counter] : operations_finished) {
    process::metrics::remove(counter);
  }

  for (const auto& [_, counter] : operations_failed) {
    process::metrics::remove(counter);
  }

  for (const auto& [_, counter] : operations_dropped) {
    process::metrics::remove(counter);
  }
}
