...
static bool isValidType(const string& s)
{
  ...
  for (const auto& token : strings::split(s, ".")) {
      ...
      if (realpath.isSome()) {
        ...
        if (os::exists(statePath)) {
          ...
          if (resourceProviderState.isSome()) {
            ...
            for (const auto& entry : storage.profiles()) {
              profileInfos.put(
                  entry.first,
            ...
            for (const auto& resource : totalResources) {
  ...
  return collect<vector<ResourceConversion>>(
    ...
    .then(defer(
        ...
        (const vector<vector<ResourceConversion>>& collected) {
          ...
          for (const auto& conversions : collected) {
    ...
    const Resources& checkpointed, const Resources& discovered) const
{
  ...
  for (const auto& resource : checkpointed) {
    ...
    const hashset<string>& profiles)
{
  ...
  for (const auto& profile : profileInfos.keys()) {
    ...
    const Event::ReconcileOperations& reconcile)
{
  ...
  for (const auto& operationUuid : reconcile.operation_uuids()) {
    ...
    const Offer::Operation& operation) const
{
  ...
  for (const auto& resource : operation.create().volumes()) {
    CHECK(Resources::isPersistentVolume(resource));
    CHECK(resource.disk().source().has_id());
    ...
    const Offer::Operation& operation) const
{
  ...
  for (const auto& resource : operation.destroy().volumes()) {
    CHECK(Resources::isPersistentVolume(resource));
    CHECK(resource.disk().source().has_id());
    ...
    const Try<vector<ResourceConversion>>& conversions)
{
  ...
  if (conversions.isSome()) {
    ...
    for (auto& conversion : conversions.get()) {
  ...
  : csi::Metrics(prefix)
{
  ...
  for (const auto& type : operationTypes) {
