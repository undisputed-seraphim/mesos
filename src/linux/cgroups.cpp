...
static Try<Nothing> mount(const string& hierarchy, const string& subsystems)
{
  ...
  for (const auto& subsystem : strings::tokenize(subsystems, ",")) {
...
Result<string> hierarchy(const string& subsystems)
{
  ...
  for (const auto& candidate : hierarchies.get()) {
    if (subsystems.empty()) {
      hierarchy = candidate;
      break;
    }
...
Try<set<string>> subsystems()
{
  ...
  set<string> names;
  for (const auto& [_, info] : infos.get()) {
    if (info.enabled) {
    ...
    bool recursive)
{
  ...
  if (attached.isError()) {
    ...
    for (const auto& cgroup : missingCgroups) {
...
private:
  ...
  Future<Nothing> kill()
  {
    ...
    // correct pids.
    for (const auto pid : processes.get()) {
    ...
    const string& file)
{
  ...
  for (const auto& line : strings::split(contents.get(), "\n")) {
    // Skip empty lines.
...
Result<string> cgroup(pid_t pid, const string& subsystem)
{
  ...
  for (const auto& line : strings::tokenize(read.get(), "\n")) {
    ...
    for (const auto& token : strings::tokenize(tokens[1], ",")) {
      if (subsystem == token) {
        cgroup = tokens[2];
      }
    }
    ...
    const string& control)
{
  ...
  for (const auto& s : strings::tokenize(read.get(), "\n")) {
