...
Try<bool> supported(const string& fsname)
{
  ...
  for (const auto& line : strings::tokenize(lines.get(), "\n")) {
    ...
    bool hierarchicalSort)
{
  ...
  for (const auto& line : strings::tokenize(lines, "\n")) {
    ...
    const std::string& target)
{
  ...
  // 'realTarget'.
  for (const auto& entry : adaptor::reverse(entries)) {
...
{
  for (const auto& token : strings::tokenize(optionalFields, " ")) {
