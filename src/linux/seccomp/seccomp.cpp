    ...
    const Option<ProcessCapabilities>& capabilities)
{
  ...
  // Add architectures.
  for (auto arch : profile.architectures()) {
  ...
  foreach (
    ...
    if (capabilities.isSome()) {
      ...
      if (syscall.has_includes() &&
        ...
        for (auto capability : syscall.includes().capabilities()) {
      ...
      if (syscall.has_excludes() &&
        ...
        for (auto capability : syscall.excludes().capabilities()) {
    ...
    for (const auto& name : syscall.names()) {
