    ...
    ContainerSeccompProfile* profile)
{
  ...
  for (const auto& item : archMap->values) {
    ...
    for (const auto& subItem : subArchitectures->values) {
    ...
    bool* architectureMatched)
{
  ...
  if (arches.isSome()) {
    ...
    for (const auto& item : arches->values) {
  ...
  if (caps.isSome()) {
    for (const auto& item : caps->values) {
      if (!item.is<JSON::String>()) {
  ...
  ContainerSeccompProfile* profile)
{
  ...
  // of system calls.
  for (const auto& item : syscalls->values) {
    if (!item.is<JSON::Object>()) {
    ...
    for (const auto& namesItem : names->values) {
    ...
    if (args->is<JSON::Array>()) {
      for (const auto& argsItem : args->as<JSON::Array>().values) {
