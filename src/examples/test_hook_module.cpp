...
public:
      ...
      const SlaveInfo& slaveInfo) override
  {
    ...
    for (const auto& oldLabel : taskInfo.labels().labels()) {
        ...
        const Option<map<string, string>>& env) override
  {
    ...
    if (taskInfo.isSome()) {
      for (const auto& label : taskInfo->labels().labels()) {
      ...
      const TaskStatus& status) override
  {
    ...
    // Remove label which was set by test.
    for (const auto& oldLabel : status.labels().labels()) {
      if (oldLabel.key() != "foo") {
        labels.add_labels()->CopyFrom(oldLabel);
      }
    }
      ...
      const SlaveInfo& slaveInfo) override
  {
    ...
    for (const auto& resource : slaveInfo.resources()) {
