    ...
    bool rebooted)
{
  ...
  for (const auto& path : frameworks.get()) {
  ...
  if (os::exists(targetPath)) {
    ...
    if (resourceState.isSome()) {
      state.operations = std::vector<Operation>();
      for (const auto& operation : resourceState->operations()) {
        state.operations->push_back(operation);
      }
    }
  }
    ...
    bool rebooted)
{
  ...
  for (const auto& path : executors.get()) {
