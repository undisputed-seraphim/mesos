...
static Try<Nothing> prepareMounts(const ContainerLaunchInfo& launchInfo)
{
  ...
  if (!hasSharedMount) {
    ...
    for (const auto& mount : launchInfo.mounts()) {
      // Skip normal mounts.
...
#else
  for (const auto& limit : limits.rlimits()) {
...
int MesosContainerizerLaunch::execute()
{
  ...
  for (const auto& target : launchInfo.masked_paths()) {
  ...
  for (const auto& op : launchInfo.file_operations()) {
  ...
  for (const auto& command : launchInfo.pre_exec_commands()) {
  ...
  if (launchInfo.has_user()) {
    ...
    if (_uid.get() != ::geteuid()) {
      ...
      for (auto supplementaryGroup : launchInfo.supplementary_groups()) {
