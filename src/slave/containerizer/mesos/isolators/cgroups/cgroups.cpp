...
Try<Isolator*> CgroupsIsolatorProcess::create(const Flags& flags)
{
  ...
  for (const auto& [_, subsystemName] : isolatorMap) {
  ...
  if (strings::contains(flags.isolation, "cgroups/all")) {
    ...
    for (const auto& subsystemName : enabledSubsystems.get()) {
    ...
    for (auto isolator : strings::tokenize(flags.isolation, ",")) {
      ...
      for (const auto& subsystemName : isolatorMap.get(isolator)) {
        subsystemSet.insert(subsystemName);
      }
    }
  }
  ...
  for (const auto& subsystemName : subsystemSet) {
    ...
    const vector<Future<Nothing>>& futures)
{
  ...
  for (const auto& hierarchy : subsystems.keys()) {
    ...
    const ContainerID& containerId)
{
  ...
  for (const auto& hierarchy : subsystems.keys()) {
    ...
    for (const auto& subsystem : subsystems.get(hierarchy)) {
    ...
    pid_t pid)
{
  ...
  if (infos.contains(containerId)) {
    for (const auto& [_, subsystem] : subsystems) {
