    ...
    const string& cgroup)
{
  ...
  if (version.get() < Version(5, 0, 0)) {
    ...
    for (const auto& value : sectors.get()) {
      if (value.device.isNone()) {
    ...
    for (const auto& statValue : io_service_bytes.get()) {
    ...
    for (const auto& statValue : io_serviced.get()) {
      CgroupInfo::Blkio::Value* value = statValue.device.isSome()
    ...
    for (const auto& statValue : io_service_time.get()) {
      CgroupInfo::Blkio::Value* value = statValue.device.isSome()
    ...
    for (const auto& statValue : io_wait_time.get()) {
      CgroupInfo::Blkio::Value* value = statValue.device.isSome()
    ...
    for (const auto& statValue : io_merged.get()) {
      CgroupInfo::Blkio::Value* value = statValue.device.isSome()
    ...
    for (const auto& statValue : io_queued.get()) {
      CgroupInfo::Blkio::Value* value = statValue.device.isSome()
