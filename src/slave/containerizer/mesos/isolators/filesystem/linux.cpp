    ...
    const ContainerConfig& containerConfig)
{
  ...
  if (containerConfig.has_rootfs()) {
    ...
    *launchInfo.add_mounts() = createContainerMount(
    ...
    for (const auto& mnt : ROOTFS_CONTAINER_MOUNTS) {
    ...
    *launchInfo.add_mounts() = createContainerMount(
    ...
    if (!privileged.get()) {
      for (const auto& path : ROOTFS_MASKED_PATHS) {
  ...
  return update(containerId, containerConfig.resources())
    .then(defer(
        ...
        [this, containerId, containerConfig, launchInfo]() mutable
          ...
          for (auto& gid : infos[containerId]->gids) {
    ...
    const google::protobuf::Map<string, Value::Scalar>& resourceLimits)
{
  ...
  for (const auto& resource : current.persistentVolumes()) {
