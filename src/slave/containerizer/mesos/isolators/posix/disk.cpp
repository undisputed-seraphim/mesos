    ...
    const hashset<ContainerID>& orphans)
{
  for (const auto& state : states) {
    ...
    for (const auto& path : state.ephemeral_volumes()) {
    ...
    const ContainerConfig& containerConfig)
{
  ...
  for (const auto& path : containerConfig.ephemeral_volumes()) {
    info->directories.insert(path);
  }
    ...
    const google::protobuf::Map<string, Value::Scalar>& resourceLimits)
{
  ...
  for (const auto& resource : resourceRequests) {
  ...
  if (quotas.contains(info->sandbox)) {
    ...
    for (const auto& path : info->directories) {
      quotas[path] = quota;
    }
  }
  ...
  for (const auto& path : info->paths.keys()) {
    ...
    const Future<Bytes>& future)
{
  ...
  if (future.isReady()) {
    ...
    if (flags.enforce_container_disk_quota) {
      ...
      for (const auto& resource : info->paths[path].quota) {
...
private:
  ...
  void schedule()
  {
    ...
    for (const auto& exclude : entry->excludes) {
