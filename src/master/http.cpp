    ...
    const Option<Principal>& principal) const
{
  ...
  for (const auto& value : parse->values) {
    ...
    const Owned<ObjectApprovers>& approvers) const
{
  ...
  foreach (const mesos::maintenance::Window& window,
    ...
    for (const auto& machine_id : window.machine_ids()) {
    ...
    const Owned<ObjectApprovers>& approvers) const
{
  for (const auto& window : schedule.windows()) {
    for (const auto& machine : window.machine_ids()) {
    ...
    bool applied) const
{
  ...
  for (const auto& window : schedule.windows()) {
    for (const auto& id : window.machine_ids()) {
    ...
    foreachpair (
      ...
      switch (machine.info.mode()) {
        case MachineInfo::DRAINING: {
          ...
          for (const auto& slave : machine.slaves) {
  ...
  if (slave != nullptr) {
    ...
    if (!master->maintenance.schedules.empty()) {
      foreach (
          ...
          master->maintenance.schedules.front().windows()) {
        for (const auto& machineId : window.machine_ids()) {
