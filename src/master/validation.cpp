...
Option<Error> registerSlave(const RegisterSlaveMessage& message)
{
  ...
  for (const auto& resource : message.checkpointed_resources()) {
...
Option<Error> reregisterSlave(const ReregisterSlaveMessage& message)
{
  ...
  for (const auto& framework : message.frameworks()) {
  ...
  for (const auto& executor : message.executor_infos()) {
...
{
  for (const auto& name : _resources.names()) {
    ...
    const vector<Resources>& resources)
{
  ...
  for (const auto& resources_ : resources) {
    ...
    const Option<FrameworkInfo>& frameworkInfo)
{
  ...
  for (const auto& resource : reserve.resources()) {
    if (!Resources::isDynamicallyReserved(resource)) {
      return Error(
    ...
    const Option<FrameworkInfo>& frameworkInfo)
{
  ...
  for (const auto& resource : unreserve.resources()) {
    if (!Resources::isDynamicallyReserved(resource)) {
      return Error(
          "Resource " + stringify(resource) + " is not dynamically reserved");
    }
    ...
    const Option<FrameworkInfo>& frameworkInfo)
{
  ...
  for (const auto& volume : create.volumes()) {
